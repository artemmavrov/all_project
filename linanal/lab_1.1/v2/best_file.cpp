...
map<int, long double> get_vertical(Matrix& A, int j) {
	map<int, long double> vertical;
...
map<int, long double> get_horizontal(Matrix& A, int i) {
	map<int, long double> horizontal;
...
long double multiply_vectors(map<int, long double>& a, map<int ,long double>& b) {
	long double res = 0;

	for(auto element : a) {
		if(b.count(element.first) != 0) {
			res += element.second * b[element.first];
		}
	}
...
Matrix multiply_matrix(Matrix& A, Matrix& B) {
	...
	for (int i = 0; i < new_n; i++)
	{
		...
		map<int, long double> horizontal = get_horizontal(A, i);
		for (int j = 0; j < new_m; j++)
		{
			map<int, long double> vertical = get_vertical(B, j);
